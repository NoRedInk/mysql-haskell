...
dumpBinLog ::
  ...
  -- | binlog position
  BinLogTracker ->
  -- | if master support semi-ack, do we want to enable it?
...
dumpBinLog conn@(MySQLConn is wp _ consumed) sid (BinLogTracker initfn initpos) wantAck = do
  ...
  es <- Stream.makeInputStream $ do
    ...
    case q of
      ...
      Just q' -> do
        when (blEventType q' == BINLOG_ROTATE_EVENT) $ do
              ...
              | t == BINLOG_TABLE_MAP_EVENT -> do
                ...
                case q of
                  ...
                  Just q' -> do
                    let u = blEventType q'
                    if
                        | u == BINLOG_WRITE_ROWS_EVENTv1 || u == BINLOG_WRITE_ROWS_EVENTv2 -> do
                          tr <- track q' fref
