...
data BinLogValue
  ...
  | -- | a 64bit bitmap.
    BinLogBit !Word64
  | -- | a utc timestamp, note 0 doesn't mean @1970-01-01 00:00:00@,
...
getBinLogField (BINLOG_TYPE_NEWDECIMAL precision scale) = do
  ...
  let fb = buf `B.unsafeIndex` 0
      ...
      iPart =
        fromIntegral (getCompressed cISize (B.unsafeTake cISize buf'')) * (blockSize ^ ucI)
          + getUncompressed ucI (B.unsafeDrop cISize buf'')

  let buf''' = B.unsafeDrop (ucISize + cISize) buf''

      fPart =
        getUncompressed ucF (B.unsafeTake ucFSize buf''') * (10 ^ cF)
...
getBinLogField (BINLOG_TYPE_STRING size) = do
  len <-
    if
        | size < 256 -> fromIntegral <$> getWord8
  ...
  where
    ...
    go (f : fs) nullmap nullpos pmap' ppos = do
      ...
      if isColumnSet pmap' ppos
        then do
          r <-
            if isColumnSet nullmap nullpos
