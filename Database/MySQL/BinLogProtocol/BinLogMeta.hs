...
data BinLogMeta
  ...
  | -- | 1 or 2('Word8' or 'Word16'), enum index size
    BINLOG_TYPE_ENUM !Word8
  | -- | bitmap bits, bytes
    BINLOG_TYPE_SET !Word16 !Word8
  | -- | length size
    BINLOG_TYPE_BLOB !Word8
  | -- | meta length(if < 256, then length is 8bit,
  ...
  | t == mySQLTypeString = do
    ...
    if byte0 > 0
      then
        if (byte0 .&. 0x30) /= 0x30
          then
            if FieldType (byte0 .|. 0x30) == mySQLTypeString
              then
                let len = fromIntegral $ (byte0 .&. 0x30) `xor` 0x30
                    len' = len `shiftL` 4 .|. fromIntegral byte1
                 in pure $! BINLOG_TYPE_STRING len'
              else
                let len = fromIntegral byte0 `shiftL` 8 :: Word16
                    len' = len .|. fromIntegral byte1
                 in pure $! BINLOG_TYPE_STRING len'
          else
            let t' = FieldType byte0
             in if
                    | t' == mySQLTypeSet ->
                      let nbits = fromIntegral byte1 `shiftL` 3
                    ...
                    | otherwise ->
                      fail $
                        ...
                        \ impossible type inside binlog string: "
                          ++ show t'
