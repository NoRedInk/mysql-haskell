  ...
  where
    ...
    loop oarr oend marr !ix !ix'
      ...
      | otherwise = do
        ...
        if
            | c >= 0xD800 && c <= 0xDBFF -> do
              let c2 = TA.unsafeIndex oarr (ix + 1)
...
escapeBytes (B.PS fp s len) = unsafeDupablePerformIO $
  withForeignPtr fp $ \a ->
      ...
      | otherwise = do
        ...
        if
            | c == 0
                || c == 39
                || c == 34 ->
              escape c b >>= loop (a `plusPtr` 1) aend -- \0 \' \"
