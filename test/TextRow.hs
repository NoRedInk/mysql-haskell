...
tests c = do
  ...
  Just vectorResults <- Stream.read is'
  ...
  assertWithResults
    ...
    ( \v -> do
        ...
        assertEqual "decode text protocol(queryVector_)" v (V.toList vectorResults)
    )
  ...
  assertWithResults
    ...
    ( \v ->
        assertEqual
          "decode text protocol with null"
          v
          [ MySQLInt32 0,
            MySQLBit bitV,
            MySQLInt8 (-128),
  ...
  assertWithResults
    ...
    ( \v ->
        assertEqual
          "roundtrip text protocol with null"
          v
          [ MySQLInt32 0,
            MySQLBit bitV,
            MySQLInt8 (-128),
  ...
  assertWithResults
    c
    "SELECT __time, __year FROM test"
    ( \v ->
        assertEqual
  ...
  assertWithResults
    c
    "SELECT __text, __blob FROM test"
    ( \v ->
        assertEqual
          "decode text protocol 3"
          v
          [ MySQLText "",
            MySQLBytes ""
          ]
