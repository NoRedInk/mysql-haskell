...
main :: IO ()
main = defaultMain $
  testCaseSteps "mysql-haskell test suit" $ \step -> do
    ...
    (_greet, c') <- connectDetail defaultConnectInfo {ciUser = "testMySQLHaskell", ciDatabase = "testMySQLHaskell"}
    _ <- execute_ c' "SET PASSWORD = PASSWORD('123456abcdefg???')"
    close c'

    let loginFailMsg =
          "ERRException (ERR {errCode = 1045, errState = \"28000\", \
